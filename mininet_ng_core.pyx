...
cdef extern from "core_namespaces.h":
    int start_node(char *node_name, char *merged_dir, char *lower_dir, char *upper_dir, char *work_dir, char *apparmor_profile, char *netns_name)

# 2. Creamos la función que Python llamará
def create_container(node_name: str, merged_dir: str, lower_dir: str, upper_dir: str, work_dir: str, apparmor_profile: str, netns_name: str) -> int:
    """
    ...
    cdef int child_pid = start_node(
        b_node_name, b_merged_dir, b_lower_dir,
        b_upper_dir, b_work_dir, b_apparmor, b_netns
    )
